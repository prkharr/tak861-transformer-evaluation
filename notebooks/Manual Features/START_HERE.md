...
The shared call transcript describes tensor initialization, patient splitting,
Transformer training and evaluation at 0:22. The core-feature experiment is
discussed at 19:33–20:31; reduced-feature and regularization experiments are
separate next steps. An earlier call at 40:03 asks to retain the existing
patients and index dates. This delivery follows that scope. Training lift is
included as explicitly requested in this conversation.

1. **01 — Prepare input.** Read the exact ordered feature list from MODEL_TYPE.FEATURES and values from MODEL_DATA. The executed output confirms 49 features. Preserve patient/date keys, labels, fractional values and missingness.
...
- Before comparing against the reference model's reported lift of 6, confirm its split, population, dates, outcome and targeting fraction. Lift here is selected response rate divided by that split's response rate. Decile 10 contains highest scores; ties use patient/date order.

Existing TEST results have already been inspected. The reference model's feature-selection population and historical feature availability have not been independently verified. Later untouched data is needed to confirm generalization. Fewer features do not guarantee lift 6 or eliminate overfitting.
